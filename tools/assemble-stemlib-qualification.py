...
def build(source, build_root, ctest_junit, output):
    ...
    semantic_path = table_root / "binary-semantic-comparison.tsv"
    ...
    semantic_rows = rows(semantic_path)
    require(len(semantic_rows) == 229 and all(
        len(row) == 5 and row[0] == binary_exceptions[index][0] and
        row[1].isdigit() and int(row[1]) > 0 and
        row[2].isdigit() and int(row[2]) > 0 and
        row[3].isdigit() and int(row[3]) > 0 and
        row[4] in {"semantic-identical", "reviewed-semantic-exception"}
    ...
    require(len(semantic_exceptions) == 1 and
            ...
            ], "binary semantic exceptions do not match the report")
